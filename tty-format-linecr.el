;;; tty-format-linecr.el --- text file backspacing, hiding lines ending in ^M,
;;; and ANSI SGR as faces
...
;;-----------------------------------------------------------------------------
;; Hide part of line before ^M

(defconst tty-format-linecr-regexp "^\\([^
]*\\)\\([^
]\\)"
  "An internal part of tty-format.el.
This regexp matches a line with a carriage return (^M) in the middle.")

;;;###autoload
(add-to-list 'format-alist
             '(linecr-hide
               "Hide lines ending in ^M."
               nil  ;; no automatic detection
               linecr-hide-decode
               linecr-hide-encode
               t
...
;;;###autoload
(defun linecr-hide-encode (beg end buffer)
  ;; checkdoc-params: (beg end buffer)
  "Sorry, cannot encode `linecr-hide' format.
This function is designed for use in `format-alist'.

There's no support for re-encoding to save a file in
`linecr-hide' format.  (But of course you can copy into another
document with a format that does support saving.)"
  (error "Sorry, `linecr-hide' format is read-only"))

;;;###autoload
(defun linecr-hide-decode (beg end)
  "Hide part of line up to a ^M between BEG and END.
This function is designed for use in `format-alist'.

There's no automatic detection of this format, because 
lines containing ^M could too easily occur in unrelated binary data.
Decode files with an explicit \\[format-decode-buffer], or see
`tty-format-guess' to try automated guessing on text files."
  ...
  (let ((inhibit-read-only t))  ;; if visiting a read-only file
    (save-excursion
      (save-restriction
        ...
        (goto-char 1)
        (let (me)
          (while (and (setq me (re-search-forward tty-format-linecr-regexp nil t)) (< (point) (point-max)))
            (facemenu-set-invisible (match-beginning 1) (match-end 1))))
        (point-max)))))
...
ANSI SGR escapes, backspace overstriking, or same-line display (^M),
then call `format-decode-buffer' to decode with `ansi-colors',
`backspace-overstrike', and/or
`linecr-hide' formats respectively.
...
remember the formats can always be decoded explicitly with
    ...
    \\[format-decode-buffer] linecr-hide
  ...
  (let ((filename (buffer-file-name)))
    (when filename
      ...
      (when (let ((case-fold-search t))
        ...
        (if (save-excursion
              (goto-char (point-min))
              (re-search-forward tty-format-linecr-regexp nil t))
            (format-decode-buffer 'linecr-hide))))))
...
(provide 'tty-format-linecr)

;;; tty-format-linecr.el ends here
